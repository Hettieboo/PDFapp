...
def enhance_document_image(img_array):
    """Enhance document image for better scanning quality"""
...
def image_to_pdf(image_file, enhance=True, perspective=True):
    """Convert image to PDF with enhancements"""
    # Read image
...
def create_download_link(file_bytes, filename):
    """Create a download link for processed file"""
            ...
            try:
                ...
                for output_file in output_files:
                    with st.expander(f"📄 {output_file['name']}", expanded=True):
                        ...
                        with col1:
                            st.metric("Original Pages", stats['original_pages'])
                        with col2:
                            st.metric("Final Pages", stats['final_pages'])
                        with col3:
